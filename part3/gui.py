...
class App(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        if self.game.choiceValidity(i, j) and self.game.checkWin()==0:
            self.game.play(i, j)
            pixmap = QPixmap("{}_PLAYER.png".format(["X", "O"][self.game.getPlayer()-1])).scaled(180, 180)
            self.labels[i][j].setPixmap(pixmap)
            self.game.changePlayer()
            buttonReply = QMessageBox.question(self, 'Message', "win", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
